/* =============================================================================
 * scribble-env.js — portable Babylon.js HDRI environment + scribble post-process
 *
 ...
 * scene produces would be meaningless. The environment setup installs the
 ...
 *   <script src="scribble-env.js"></script>
 *
 *   const env = ScribbleFX.createEnvironment(scene, camera, {
 *       hdr: "autumn_field_puresky_4k.hdr",
 *   });
 *   const scribble = ScribbleFX.createScribble(scene, camera, engine);
 ...
 *
 * REQUIREMENTS
 *   - Babylon.js 5+ (uses DefaultRenderingPipeline, HDRCubeTexture,
 *     createDefaultSkybox, textureCubeLodEXT handling in the shader processor)
 *   - An .hdr equirectangular file served over http (file:// will fail CORS)
 *   - WebGL2, or WebGL1 with OES_standard_derivatives
 *
 * COST
 *   One fullscreen pass, created lazily on first enable() and torn down on
 *   disable(), so nothing is paid while it is off.
 *
 * NO OUTLINES, deliberately. This shader used to darken depth discontinuities
 * (the edgeDark / outlineWidth pair), and that feature was removed rather than
 * fixed: Babylon's depth renderer draws meshes with its own depth shader, so
 * anything whose vertex shader displaces geometry — GPU-displaced rocks, the
 * water surface, foliage — leaves its UNDISPLACED silhouette in the depth map,
 * and the edge detector traced that phantom shape as a dark halo floating well
 * outside the visible object. The depth pre-pass itself stays: the ignoreSky
 * mask still reads it to hand the HDRI background back unstyled.
 * `params.outlineWidth` and `params.edgeDark` are still accepted so existing
 * callers and panels do not break, but they are inert.
 * ========================================================================== */

(function (global) {
    "use strict";

    // Babylon's shader processor rewrites the source it is handed and mishandles
    // a line comment containing a semicolon: the comment marker is dropped and
    // the tail of the sentence survives into the compiled GLSL as a syntax
    // error. Stripping comments on the way to the compiler sidesteps it, and
    // the comments stay here where they are useful.
    //
    ...
    // -------------------------------------------------------------------------
    function createEnvironment(scene, camera, opts) {
        opts = opts || {};
        const url = opts.hdr;
        if (!url) throw new Error("ScribbleFX.createEnvironment: opts.hdr is required");

        // An .hdr is LINEAR and high dynamic range: sky sits around 1-3 while
        // the sun region runs far above. Sampling it straight into an 8-bit
        // target clips every bright reflection to flat white, which is why the
        // pipeline below renders in HDR and tone-maps once at the end.
        const envTex = new BABYLON.HDRCubeTexture(url, scene, opts.cubeSize || 512);
        scene.environmentTexture = envTex;

        // PBR skybox rather than a hand-rolled StandardMaterial one: it consumes
        // the linear HDR cube correctly and leaves tone mapping to the
        // post-process, which is what keeps the sky and anything reflecting that
        // same sky on ONE curve. Two curves show up as a seam along the horizon.
        const skybox = scene.createDefaultSkybox(envTex, true, opts.skyboxSize || 1000, 0);
        ...
        // Warmth as a global grade rather than a tint on any one material, so
        // the sky and everything lit by it shift together.
    ...
    //
    // The obvious approach — draw a small random canvas and let the browser
    // scale it up — produces a texture whose left edge does not match its right.
    ...
    // frame into offset rectangles; two repeat rates overlaid give a visible
    // grid of them. Building it from a lattice indexed with modulo makes it
    // genuinely tileable, so any repeat rate stays continuous.
    // -------------------------------------------------------------------------
    function makeSeamlessNoise(scene, cells, smooth, name) {
        const S = 256;
    ...
    BABYLON.Effect.ShadersStore[FRAG_NAME + "FragmentShader"] = glsl(`
...
`);
    ...
    function createScribble(scene, camera, engine, opts) {
        opts = opts || {};
        const params = {
            // Colour bleed stays 0 — that was the camera-locked swimming. The
        ...
        function enable(v) {
            ...
            if (v) {
                ...
                if (!pp) {
                    pp = new BABYLON.PostProcess("scribbleFx", FRAG_NAME,
                    ...
                    pp.onApply = (eff) => {
                        ...
                        // Gated on the depth renderer actually existing. Sky
                        // masking reads the depth buffer, and if that is
                        // missing every pixel reports the far value — which
                        // masks out the WHOLE frame and makes the pass look
                        // silently dead rather than erroring. Fail to "style
