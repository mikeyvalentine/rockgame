...
export class Overlay {
    /**
     * @param {{ rig?: import("../core/camera.js").FpsRig,
    ...
    constructor(refs) {
        ...
        hdr.className = "hdr";
        hdr.innerHTML = "<b>SAND-SIM</b><i>F1 to close</i>";
        el.appendChild(hdr);
    ...
    attach(refs) {
        ...
        if (refs.deform) this.deform = refs.deform;
        if (refs.grains) this.grains = refs.grains;
    }
    ...
    _updateCamera() {
        ...
        this._txt(r.camArm, (rig.fov * RAD).toFixed(1) + "° v");
        ...
        // The pausable-sim gate's state — the phase-9 verification readout.
        const df = this.deform;
        if (df) {
            const grains = this.grains;
            this._txt(
                r.simState,
                (df.sleeping ? "asleep" : "stepping") +
                (grains ? "  ·  " + grains.liveCount + " grains" : "")
            );
        } else {
            this._txt(r.simState, "—");
        }
