...
fn terrainFine(p: vec2f, w: f32, exposure: f32, amp: f32) -> vec3f {
    ...
    // Beach retune: snow sastrugi stood 10-30 cm proud; wind-worked sand ridges
    // are a fifth of that. Same shape, sand's amplitude.
    let sasAmp = 0.02 * amp * mix(0.45, 1.0, exposure) * scour;
    h += (sas.x - 0.35) * sasAmp;
    d += (sas.yz * m3) * sasAmp;
...
fn terrainFineFiltered(p: vec2f, w: f32, exposure: f32, amp: f32, fp: f32) -> vec3f {
    ...
    if (fadeS > 0.001) {
        ...
        let scour = 0.45 + 0.55 * smoothstep(-0.25, 0.35, noise2(p * 0.021));
        // Must match `terrainFine`'s beach amplitude above — one is the vertex
        // displacement, the other the fragment normal.
        let a = 0.02 * amp * mix(0.45, 1.0, exposure) * scour * fadeS;
        h += (sas.x - 0.35) * a;
        d += (sas.yz * m3) * a;
    }
