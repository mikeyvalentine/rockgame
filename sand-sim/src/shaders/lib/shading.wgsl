...
fn snowSubsurface(
    ...
    // Sand retune: what little light transmits through a thin sand edge comes
    // back *warm* — grains absorb blue, the opposite of snow. A backlit berm
    // crest picks up a faint amber rim instead of a blue glow.
    let shallowTint = vec3f(1.0, 0.96, 0.88);
    let deepTint = vec3f(0.88, 0.78, 0.62);
