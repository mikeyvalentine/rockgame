...
//
// Channels (sand semantics — A repurposed from SNOWFLOW's ice):
...
//   B  compaction 0..1 — trodden sand: denser, darker, tighter specular
//   A  wetness 0..1 — damp sand (dug up from depth, splashed). Max-combined on
//      write, dries slowly, and slows the relaxation of the texels it covers —
//      wet sand holds its shape.
//
...
fn main(input: FragmentInputs) -> FragmentOutputs {
    ...
    if (wasInside) {
        ...
        // Wet sand holds its shape: damp texels relax far slower than dry
        // ones. This is what keeps prints near the waterline crisp while the
        // dry upper beach slowly softens. (`ice` is the wetness channel.)
        let wetK = 1.5 - 1.2 * clamp(ice, 0.0, 1.0);
        let kDep = min(0.22, 0.004 * k * wetK);
        let kBerm = min(0.22, 0.012 * k * wetK);
        ...
        // Damp sand does not blow.
        let kAdv = min(0.2, 0.002 * k) * (1.0 - clamp(ice, 0.0, 1.0) * 0.85);
        ...
        // 3x the snow rate: dry sand's angle of repose is low and loose piles
        // give it up quickly. (Wet modulation comes via the diffusion above —
        // slump conserves mass either way.)
        ...
        // mean what they say.
        // Sand barely self-heals: there is no snowfall refilling a beach, and
        // the game docs *want* trails to persist. An order of magnitude slower
        // than the snow constants — footprints survive the session; only the
        // slow diffusion above visibly softens them. (Still banked through the
        // same half-float ULP reasoning; longer constants make banking *more*
        // necessary, not less.)
        let r = uniforms.refillRate;
        dep *= exp(-dt * r / 4000.0);
        berm *= exp(-dt * r / 2500.0);
        comp *= exp(-dt * r / 3000.0);
        // Wetness dries in minutes — dug-up damp sand pales as you watch.
        ice *= exp(-dt * r / 600.0);
    }
