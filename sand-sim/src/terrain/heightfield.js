...
// Sizes live in beachParams.js (shared with the WebGL path and the headless
// checks); re-exported here so terrain.js keeps one import site.
export { WORLD_SIZE, HEIGHT_RES, AUX_RES };
...
export class Heightfield {
    ...
    async bake() {
        ...
        this.heightTex.setFloat("bermRelax", BERM_RELAX);
        this.heightTex.setFloat("duneStart", DUNE_START);
        this.heightTex.setFloat("duneFade", DUNE_FADE);
        this.heightTex.setFloat("duneAmp", DUNE_AMP);
        this.heightTex.setFloat("duneBase", DUNE_BASE);
        this.heightTex.setFloat("microAmp", MICRO_AMP);
        ...
        this.auxTex.setVector2("worldOrigin", this.origin);
        this.auxTex.setFloat("worldSize", this.size);
        this.auxTex.setFloat("pebbleBandCenter", PEBBLE_BAND_CENTER_Z);
        this.auxTex.setFloat("pebbleBandWidth", PEBBLE_BAND_WIDTH);
    ...
    /**
     * Clamp a world position to the walkable beach rect, in place. Rectangular
     * (docs/09: a bounded coastal strip), replacing SNOWFLOW's radial clamp.
     */
    clampToPlayArea(v) {
        clampToPlayRect(v);
    }
