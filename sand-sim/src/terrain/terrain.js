...
import { WATERLINE_Z } from "./beachParams.js";
import {
    SAND_DRY, SAND_WET, PEBBLE_TINT, WET_NEAR, WET_FAR,
} from "../looks/sandTone.js";
...
export class Terrain {
    ...
    _makeSnowMaterial() {
        const mat = new ShaderMaterial(
            ...
            { vertex: "snow", fragment: "snow" },
            {
                ...
                uniforms: [
                    ...
                    "sandDry", "sandWet", "waterlineZ", "wetNear", "wetFar",
                    "maskOrigin", "maskSize", "pebbleTint",
                ],
                samplers: [
                    "heightTex", "auxTex", "detailTex", "skyLUT",
                    "cascade0", "cascade1", "cascade2", "deformTex", "maskTex",
                ],
                shaderLanguage: ShaderLanguage.WGSL,
            }
        );
        ...
        // The sand palette + shore-wetness band. Static — set once, from the
        // shared tone module, so every renderer retunes from one file.
        mat.setColor3("sandDry", new Color3(SAND_DRY.r, SAND_DRY.g, SAND_DRY.b));
        mat.setColor3("sandWet", new Color3(SAND_WET.r, SAND_WET.g, SAND_WET.b));
        mat.setColor3("pebbleTint", new Color3(PEBBLE_TINT.r, PEBBLE_TINT.g, PEBBLE_TINT.b));
        mat.setFloat("waterlineZ", WATERLINE_Z);
        mat.setFloat("wetNear", WET_NEAR);
        mat.setFloat("wetFar", WET_FAR);
        return mat;
    }
    ...
    /**
     * Bind the world-anchored paint mask. MUST be called before `warmUp()` —
     * the sand material declares the sampler, and an unbound sampler leaves
     * `isReady` false forever, which hangs the loading screen.
     * @param {import("./maskPaint.js").MaskPaint} maskPaint
     */
    attachMask(maskPaint) {
        this.material.setTexture("maskTex", maskPaint.texture);
        this.material.setVector2("maskOrigin", maskPaint.origin);
        this.material.setFloat("maskSize", maskPaint.size);
    }
