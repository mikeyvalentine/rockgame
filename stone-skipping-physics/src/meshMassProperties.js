...
export function massProperties(positions, indices, density = 1) {
  ...
  if (degenerate) {
    return {
      volume: 0, mass: 0, com: { x: 0, y: 0, z: 0 }, degenerate: true, windingFlipped: false,
      inertia: { xx: 0, yy: 0, zz: 0, xy: 0, xz: 0, yz: 0 },
    };
  }
  // Recorded because callers building surface panels need it: a mesh wound
  // inside-out yields outward normals pointing IN, and the ventilation weighting
  // keys off normal direction.
  const windingFlipped = vol < 0;
...
export function shapeDescriptors(positions, indices, density = 1) {
  ...
  // Silhouette area looking down the face axis (body Y). For a CLOSED surface the
  // projection is covered exactly twice — once by the panels facing the viewer, once
  // by those facing away — so half the |n·y|-weighted area is the true outline area.
  // Exact for any closed mesh, and far better than pretending the face is an ellipse.
  let faceArea = 0;
  for (let f = 0; f < indices.length; f += 3) {
    const ia = indices[f] * 3, ib = indices[f + 1] * 3, ic = indices[f + 2] * 3;
    ...
    // The y component of the (unnormalised) edge cross product is already twice this
    // triangle's projected area on XZ, so nothing needs normalising.
    faceArea += Math.abs(e1z * e2x - e1x * e2z) / 2;
  }
  faceArea /= 2;

  return {
    ...mp,
    flatness, asymmetry, lopsidedness, moments, axes,
    span: [...span].sort((x, y) => y - x),
    extent: { x: span[0], y: span[1], z: span[2] },
    bboxCentre: { x: centre[0], y: centre[1], z: centre[2] },
    faceArea,
  };
}

/**
 * Rotate a mesh so its natural spin axis becomes +Y, returning new positions.
 *
 * The solver takes a mesh in the orientation it is given, because for a warped stone
 * the face the player grips and the rock's principal axis genuinely disagree, and
 * that disagreement IS the wobble. Use this only when a mesh arrives in arbitrary
 * orientation (a raw scan) and needs a sane starting frame — it aligns the
 * MAXIMUM-inertia axis to +Y, which for anything flat is the face normal.
 *
 * Note this necessarily zeroes the products of inertia, since the body frame becomes
 * the principal frame. Wobble then comes from the two transverse moments differing
 * (`asymmetry`) and from the centre-of-mass offset, rather than from misalignment.
 * Output is centred on the centre of mass.
 */
export function alignMeshToFaceAxis(positions, indices, density = 1) {
  const mp = massProperties(positions, indices, density);
  if (mp.degenerate) return Float64Array.from(positions);
  const { axes } = principalAxes(mp.inertia);
  const yA = axes[0], xA = axes[1], zA = axes[2];
  const out = new Float64Array(positions.length);
  for (let i = 0; i < positions.length; i += 3) {
    const x = positions[i] - mp.com.x;
    const y = positions[i + 1] - mp.com.y;
    const z = positions[i + 2] - mp.com.z;
    out[i] = x * xA.x + y * xA.y + z * xA.z;
    out[i + 1] = x * yA.x + y * yA.y + z * yA.z;
    out[i + 2] = x * zA.x + y * zA.y + z * zA.z;
  }
  return out;
}
