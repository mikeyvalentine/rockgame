...
//   forgeRocks.js   stones, generated by rock-forge into archetypes
...
async function boot() {
  ...
  // Every stone in the scene is generated. The scanned GLB it replaced was five
  // silhouettes and 14 MB, so a bed was those five at assorted scales, and a rock's
  // appearance had nothing to do with its physics because the two were authored
  // separately.
  //
  // The generator that once lost to the scans is not this one: that was
  // noise-displaced icospheres in this lab. This is rock-forge — superellipsoid
  // base, facet carving, wear passes, per-lithology archetypes — and it hands the
  // SAME geometry to the renderer, the collision hull, the rating and the skip
  // solver, so a stone that looks warped is warped everywhere it matters.
